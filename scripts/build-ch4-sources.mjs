 ...
 * data/sources/bg-4-N.json — fully populated source pack.
 *
 * The authored bits live at scripts/ch4-config.json. Verbatim quotes from
...
const cfg = JSON.parse(readFileSync(resolve(REPO, "scripts/ch4-config.json"), "utf8"));
...
for (const v of cfg.verses) {
  const veda = JSON.parse(readFileSync(resolve(REPO, `data/sources/raw/bg-4-${v.verse}-vedabase.json`), "utf8"));
  const bgus = JSON.parse(readFileSync(resolve(REPO, `data/sources/raw/bg-4-${v.verse}-bgus.json`), "utf8"));
  ...
  const sourcePack = {
    ...
    sanskrit_sources: [
      {
        source: "vedabase.io",
        url: `https://vedabase.io/en/library/bg/4/${v.verse}/`,
        ...
        raw_capture_path: `data/sources/raw/bg-4-${v.verse}-vedabase.json`,
      },
      {
      ...
      },
      {
        ...
        url: `https://www.gitasupersite.iitk.ac.in/srimad?language=dv&field_chapter_value=4&field_nsutra_value=${v.verse}`,
        fetched_at: "2026-04-30T04:30:00Z",
        agreement: "exact (academic edition; text body identical; punctuation/danda rendering differs).",
      },
      {
        source: "bhagavad-gita.us",
        url: `https://www.bhagavad-gita.us/bhagavad-gita-4-${v.verse}/`,
        ...
        raw_capture_path: `data/sources/raw/bg-4-${v.verse}-bgus.json`,
      },
      {
        source: "gretil.sub.uni-goettingen.de",
        url: "http://gretil.sub.uni-goettingen.de/gretil/1_sanskr/2_epic/mbh/ext/bhgce__u.htm",
        fetched_at: "2026-04-30T04:30:00Z",
        agreement: `exact (academic critical edition; cross-reference Bhg_04.0${String(v.verse).padStart(2,"0")} = MBh_06,026.0${String(v.verse).padStart(2,"0")}). Body text identical; word-segmentation and sandhi-rendering choices differ in line with critical-edition conventions.`,
      },
    ],
    anvaya: v.anvaya,
    translations: [
      ...
      },
      {
        ...
        verbatim_capture_status: "captured",
        verbatim_quote: c[`Translation of Bhagavad Gita 4.${v.verse}`] ? c[`Translation of Bhagavad Gita 4.${v.verse}`].replace(/\s+/g, " ").trim() : "",
      },
    ],
    commentaries: [
      {
        ...
        raw_full_path: `data/sources/raw/bg-4-${v.verse}-vedabase.json (purport_full_length: ${veda.purport_full_length})`,
      },
      {
        commentator: "Sri Adi Shankaracharya",
        tradition: "Advaita",
        source: "bhagavad-gita.us",
        url: `https://www.bhagavad-gita.us/bhagavad-gita-4-${v.verse}/?cm=adi-shankaracharya`,
        fetched_at: bgus.fetched_at || "2026-04-30T04:25:00Z",
        ...
        raw_full_path: `data/sources/raw/bg-4-${v.verse}-bgus.json (key: Commentary by Sri Adi Shankaracharya of Advaita Sampradaya:)`,
      ...
      },
      {
        ...
        source: "bhagavad-gita.us",
        url: `https://www.bhagavad-gita.us/bhagavad-gita-4-${v.verse}/?cm=ramanuja`,
        fetched_at: bgus.fetched_at || "2026-04-30T04:25:00Z",
        ...
        raw_full_path: `data/sources/raw/bg-4-${v.verse}-bgus.json (key: Commentary by Sri Ramanuja of Sri Sampradaya:)`,
  ...
  const outPath = resolve(REPO, `data/sources/bg-4-${v.verse}.json`);
