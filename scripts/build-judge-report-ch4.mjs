...
const verseDisplayId = `BG 4.${verseNum}`;
...
for (const [persona, gateIds] of Object.entries(PERSONA_GATES)) {
  ...
  for (const gid of gateIds) {
    results.push({
      ...
      verdict,
      raw: {
        rendered_prompt_path: `data/judge-runs/${verseId}/${persona}.prompt.txt`,
        verdict_path: `data/verses/${verseId}.judge-results/${persona}.json`,
